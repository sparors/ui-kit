...
<button x-data="{{ json_encode(['isOn' => old($valueName ?? $name, $isOn) == ($on ?? true), 'on' => $on ?? true, 'off' => $off ?? false, 'input' => '']) }}" x-init="$watch('isOn', value => input = value == on ? on : off)" x-on:click="isOn = !isOn" x-bind:class="{ 'bg-primary-600': isOn, 'bg-gray-200': !isOn }" type="button" aria-pressed="false" class="bg-gray-200 relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
